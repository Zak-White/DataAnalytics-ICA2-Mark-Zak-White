...
TimeLineBudget_df <- inner_join(timelines_df, projects_df2,
                                by = c("ProjectID"))
TimeLineBudget_df
TimeLineBudget_df<- TimeLineBudget_df %>% group_by(ProjectID) %>% 
mutate(ActualCompletionDate = ymd(ActualCompletionDate),ExpectedCompletionDate = ymd(ExpectedCompletionDate)) %>% 
mutate(DifferenceInDays=as.numeric(difftime(ActualCompletionDate, ExpectedCompletionDate, units = "days")))
