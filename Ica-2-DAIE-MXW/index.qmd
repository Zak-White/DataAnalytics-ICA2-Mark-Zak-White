...
TimeLineBudget_df <- inner_join(timelines_df, projects_df2,
                                by = c("ProjectID"))
TimeLineBudget_df<- TimeLineBudget_df %>% group_by(ProjectID) %>% mutate(ActualCompletionDate = ymd(ActualCompletionDate),ExpectedCompletionDate = ymd(ExpectedCompletionDate)) %>% mutate(DifferenceInDays=as.numeric(difftime(ExpectedCompletionDate, ActualCompletionDate, units = "days")))
TimeLineBudget_df <- TimeLineBudget_df[,-c(1,3,4,5,6,7,8,10,11)]
TimeLineBudget_df <- TimeLineBudget_df[1:4,]
TimeLineBudget_df
```
